...
const Navbar = () => {
  return (
    <div className="max-w-screen-md mx-auto flex items-center justify-between p-3 py-4 border-b-[1px] ">
      <div className="flex items-center">
        <div className="flex w-40 items-center gap-2">
          ...
          <h1 className="text-sm tracking-wide font-semibold">Refokus</h1>
        </div>
        <div className="flex gap-8">
          {["Home", "Work", "Culture", "", "News"].map((elem, i) =>
            elem.length == 0 ? (
              <span key={i} className="w-[0.5px] h-[18px] mt-0.8 bg-zinc-500"></span>
            ) : (
              <a className="text-sm flex items-center" key={i} href="#">
                {i == 1 && <span style={{boxShadow:"0 0 0.25em #00FF19"}} className="inline-block w-1 h-1 bg-lime-300 mr-1 rounded-full"></span>}
                {elem}
              </a>
            )
          )}
